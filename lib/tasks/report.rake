...
namespace :toppy_lesson_report do
  desc 'Handling respond data from classin'
  task :handling_classin_data, [:start_date, :end_date] => :environment do |t, args|
    start_time = Time.parse(args[:start_date]).beginning_of_day
    end_time = Time.parse(args[:end_date]).end_of_day
